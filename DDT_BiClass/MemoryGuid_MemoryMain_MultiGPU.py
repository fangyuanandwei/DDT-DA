...
def train_epoch(args, epoch_idx, train_dataset, model, optimizer, MemoryGuid):
    ...
    for batch_idx, data in enumerate(train_dataset):
        ...
        if args.initepoch:
            if epoch_idx == 1:
                softmax_loss, feature_loss = MemoryGuid.module.predict(feature_target, output_t1, output_t2, index_target)
...
if __name__ == '__main__':
    ...
    save_dir = './MemoryResult_VisDA_MultiGPU/Memory_{}_{}_{}_{}/Classification_{}_{}'.format(
    ...
    '''Init Model'''
    Genera = Generator(base_net='ResNet101')
    class1 = Classifier(number_classes=args.num_class)
    class2 = Classifier(number_classes=args.num_class)
    ...
    Genera = torch.nn.DataParallel(Genera).cuda()
    class1 = torch.nn.DataParallel(class1).cuda()
    class2 = torch.nn.DataParallel(class2).cuda()

    MemoryGuid.module.init_memory(Genera, target_test_loader)
