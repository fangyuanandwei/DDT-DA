...
class Memory(nn.Module):
    ...
    def feature_smiliarity(self,target_feature,index_target):
        ...
        return Fc1memory, Fc2memory, FeatureMostSimilarity_index
    ...
    def softmax_smiliarity(self, fc1, fc2, index_target):
        ...
        Fc1MostSimilarity_distance, Fc1MostSimilarity_index = torch.topk(SoftmaxFc1Distance_matrix, self.top_number + 1,dim=1,largest=False)
        ...
        Fc2MostSimilarity_distance, Fc2MostSimilarity_index = torch.topk(SoftmaxFc2Distance_matrix, self.top_number + 1,dim=1,largest=False)
    ...
    def predict(self, target_feature,fc1,fc2,index_target):
        ...
        Fc1memory, Fc2memory, FeatureMostSimilarity_index = self.feature_smiliarity(target_feature, index_target)
        loss_softmax = self.Discrepancy(Fc1memory, Fc2memory)#L1损失

        ''' Most simility loss:Center loss and '''
        loss_center = self.CloseFeatureDistance(target_feature, FeatureMostSimilarity_index)
        ...
        return loss_softmax, loss_feature, loss_center

    def GlobalMemoryLoss(self):
        '''
        Calcuate the Global fc distribute distance
        '''
        GlobalSoftmaxLoss = self.Discrepancy(self.target_softmaxFc1memory, self.target_softmaxFc2memory)
        return GlobalSoftmaxLoss

    def CloseFeatureDistance(self, target_feature, FeatureMostSimilarity_index):
        '''
        Calcuate the center distance of smility feature  to make the feature cluster together.
        '''
        FeatureMostSimilarity_otherindex = FeatureMostSimilarity_index[:,1:]
        criterion = nn.MSELoss()
        batchsize,number = FeatureMostSimilarity_index.shape
        close_centers = torch.zeros(batchsize,self.feature_dim).cuda()

        for index, close_index in enumerate(FeatureMostSimilarity_index):
            close_feature = self.target_featurememory[close_index]
            close_center = torch.sum(close_feature,dim=0) / number
            close_centers[index,:] = close_center

        center_loss = criterion(target_feature,close_centers)
        return center_loss
