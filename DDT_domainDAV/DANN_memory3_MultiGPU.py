...
sys.path.append('.')
'''Memory'''
from Memory.memory3 import Memory

'''DANN'''
import models as models

from utils.domain_discriminator import DomainDiscriminator
from model.dann import DomainAdversarialLoss, ImageClassifier
...
parser.add_argument('--method',type=str, default = 'DANN_memory', help='DANN or DANN_memory')
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        '''记忆模块,输出记忆损失'''
        if epoch == 0:
            Memory.module.update_center(feature_s, labels_s, None)
            Memory.module.featuredistance2center(feature_s,labels_s)
            Memory.module.sourcefeature2differertcenter(feature_s,labels_s)
            memoryloss = torch.tensor(0).cuda()
        else:
            Memory.module.update_center(feature_s, labels_s, None)
            Memory.module.featuredistance2center(feature_s, labels_s)
            Memory.module.sourcefeature2differertcenter(feature_s, labels_s)
            memoryloss = Memory.module.feature_smiliarity(image_t, index_t, labels_t, classifier)

        cls_loss = F.cross_entropy(fc_s, labels_s)
        transfer_loss = domain_adv(feature_s, feature_t)
...
def initmemory(source_test_loader,classifier, MemoryGuid):
    ...
    for batch_idx, (test_data, test_label, test_index) in enumerate(source_test_loader):
        ...
        MemoryGuid.module.update_center(source_feature, test_label, None)
...
if __name__ == '__main__':
    ...
    save_dir = './Memory3_banchmark_MultiGPU/DANN_memory3/Memory_{}_{}_{}_{}/save/Classification_{}_{}'.format(
    ...
    log_dir = './Memory3_banchmark_MultiGPU/DANN_memory3/Memory_{}_{}_{}_{}/log/Classification_{}_{}'.format(
        args.dataset, args.method,
        args.day, args.time,
        args.source, args.target
    )
    ...
    classifier = ImageClassifier(backbone, args.num_class)
    domain_discri = DomainDiscriminator(in_feature=classifier.features_dim, hidden_size=1024)
    ...
    '''初始化记忆模块'''
    MemoryGuid = Memory(class_number=args.num_class, feature_dim = 256, target_number = num_target, source_number = num_source, top_number = args.topK )
    
    MemoryGuid = torch.nn.DataParallel(MemoryGuid).cuda()
